...
function Checkout(){
    ...
    const {items} =  useAppSelector((store:any)=>store.cart)
    const {khaltiUrl,status} =  useAppSelector((store:any)=>store.orders)
    console.log(khaltiUrl, "URLLL")
    const total = items.reduce((total:any,item:any)=>item.Product.productPrice * item.quantity + total,0)
    ...
    const handleSubmit = async (e:FormEvent<HTMLFormElement>)=>{
        e.preventDefault()
        const productData =  items.length > 0 ? items.map((item:any)=>{
            return {
