...
function MyOrder(){
    ...
    const {items} = useAppSelector((store:any)=>store.orders)
    const [searchTerm,setSearchTerm] = useState<string>("")
    console.log(searchTerm)

    const newItems = items.filter((item:any)=>item.id.toLowerCase().includes(searchTerm) || item.orderStatus?.toLowerCase().includes(searchTerm) || item.Payment?.paymentMethod.toLowerCase().includes(searchTerm) || item.totalAmount == parseInt(searchTerm))
...
<div>
  ...
  <div className="relative flex flex-col w-full h-full overflow-scroll text-gray-700 bg-white shadow-md rounded-lg bg-clip-border">
    <table className="w-full text-left table-auto min-w-max">
      ...
      <tbody>
       {
        newItems.length > 0 && newItems.map((item:any)=>{
            return ( 
